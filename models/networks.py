...
class FiLM_BEATs(nn.Module):
    def __init__(self, input_size, feature_size):
        super(FiLM_BEATs, self).__init__()
    ...
    def forward(self, embedding, feature):
        ...
        gamma = gamma.repeat(1, 256 // 200 + 1, 1)
        beta = beta.repeat(1, 256 // 200 + 1, 1)
        # print(f'repeated_gamma: {gamma.shape}\trepeated_beta: {beta.shape}')

        gamma = gamma[:, :256, :].unsqueeze(2).repeat(1, 1, 64, 1)
        beta = beta[:, :256, :].unsqueeze(2).repeat(1, 1, 64, 1)
        # print(f'reshaped_gamma: {gamma.shape}\treshaped_beta: {beta.shape}')

        FiLMed_feature = gamma * feature + beta

        return FiLMed_feature

class FiLM_reverse_SE(nn.Module):
    def __init__(self, input_size, feature_size):
        super(FiLM_reverse_SE, self).__init__()
        self.fc_gamma = nn.Linear(input_size, feature_size)
        self.fc_beta = nn.Linear(input_size, feature_size)
    ...
    def forward(self, embedding, feature):
        ...
        feature: intermediate feature maps from the target network
        """
        # Squeeze the last dimension to match input size for Linear layer
        ...
        # Adjust gamma and beta to match the feature dimensions
        gamma = gamma.repeat(1, 256 // 512 + 1, 1)
        beta = beta.repeat(1, 256 // 512 + 1, 1)
        # print(f'repeated_gamma: {gamma.shape}\trepeated_beta: {beta.shape}')
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        if self.opt.noise_emb_type == 'BEATs':
            self.film_layers = nn.ModuleList([FiLM_BEATs(input_size=768, feature_size=ngf) for _ in range(self.opt.num_inject_layers)])
        elif self.opt.noise_emb_type == 'reverse_SE':
            self.film_layers = nn.ModuleList([FiLM_reverse_SE(input_size=512, feature_size=ngf) for _ in range(self.opt.num_inject_layers)])

        self.asp = AttentiveStatisticsPooling(channels=256)

    def forward(self, input, layers=[], encode_only=False, noise_emb_type=None, noise_emb=None, inject_layers=[]):
        if -1 in layers:
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in inject_layers and noise_emb is not None:
                    if noise_emb_type == 'BEATs':
                        feat = self.film_layers[film_idx](noise_emb, feat)
                        film_idx += 1
                    elif noise_emb_type == 'reverse_SE':
                        if layer_id == inject_layers[0]:
                            noise_emb = self.asp(noise_emb).transpose(1, 2)
                        feat = self.film_layers[film_idx](noise_emb, feat)
                        film_idx += 1
                    else:
                        pass
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in inject_layers and noise_emb is not None:
                    # print(f'feat_before: {feat.shape}\tnoise_emb: {noise_emb.shape}')
                    if noise_emb_type == 'BEATs':
                        feat = self.film_layers[film_idx](noise_emb, feat)
                        film_idx += 1
                    elif noise_emb_type == 'reverse_SE':
                        if layer_id == inject_layers[0]:
                            noise_emb = self.asp(noise_emb)
                        # print(f'pooled_noise_emb: {noise_emb.shape}')
                        feat = self.film_layers[film_idx](noise_emb, feat)
                        film_idx += 1
                    else:
                        pass
    ...
    def _resize_noise_emb(self, noise_emb, feat_size):
        # feat_height, feat_width = feat_size[2], feat_size[3]
        # noise_emb = noise_emb.unsqueeze(1).expand(-1, 256, -1, -1)
        # resized_noise_emb = F.interpolate(noise_emb, size=(feat_height, feat_width), mode='bilinear', align_corners=False)
        
        resized_noise_emb = noise_emb.view(-1)
        num_repeats = (256 * 64 * 64) // resized_noise_emb.size(0) + 1
        resized_noise_emb = resized_noise_emb.repeat(num_repeats)
        resized_noise_emb = resized_noise_emb[:256 * 64 * 64]
        resized_noise_emb = resized_noise_emb.view(1, 256, 64, 64)

        return resized_noise_emb
