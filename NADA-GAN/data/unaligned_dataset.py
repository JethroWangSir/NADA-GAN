...
class UnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.state == "Test":
            ...
            self.dir_C = os.path.join("/share/nas169/jethrowang/NADA-GAN/VoiceBank-DEMAND/trainset_for_UNA-GAN/trainC/")
            if opt.inference_set == 1:
                ...
                self.dir_C = os.path.join("/share/nas169/jethrowang/NADA-GAN/VoiceBank-DEMAND/inferenceset_for_UNA-GAN/delta/trg_nse/")
            else:
        ...
        self.C_paths = sorted(make_dataset(self.dir_C, opt.max_dataset_size))  # predicted noise from pre-trained reverse SE
        ...
        self.noise_specs = Parallel(n_jobs=self.num_cores, prefer="threads")(delayed(processInput)(i, opt.fix_w, self.spec_power, self.state, self.channels) for i in self.C_paths)
        self.no_comps_noise = Parallel(n_jobs=self.num_cores, prefer="threads")(delayed(countComps)(i) for i in self.noise_specs)
        self.noise_spec_paths = []
        self.noise_comp_dict = OrderedDict()
        for nameC, countC in zip(self.C_paths, self.no_comps_noise):
            self.noise_spec_paths += [nameC] * countC
            self.noise_comp_dict[nameC] = countC
        self.noise_specs = list(chain.from_iterable(self.noise_specs))
        self.noise_specs_len = len(self.noise_specs)
        assert self.noise_specs_len == len(self.noise_spec_paths)
        del self.no_comps_noise

        if self.state == "Test" and self.opt.random_inference == 1:
            zipped_paths = list(zip(self.B_paths, self.C_paths))
            random.shuffle(zipped_paths)
            self.shuffled_B_paths, self.shuffled_C_paths = zip(*zipped_paths)
            self.shuffled_B_paths = list(self.shuffled_B_paths)
            self.shuffled_C_paths = list(self.shuffled_C_paths)
    ...
    def __getitem__(self, index):
        ...
        if(self.state == "Train"):
            ...
            C_path = self.noise_spec_paths[index_B]
            C_img = self.noise_specs[index_B]
            C = transform(C_img)
        else:
            if self.opt.random_inference == 1:
                ...
                C_path = self.shuffled_C_paths[index_B]
                C_img = self.noise_specs[self.C_paths.index(C_path)]
                C = transform(C_img)
            else:
                index_B = index % self.noisy_specs_len
                B_path = self.noisy_spec_paths[index_B]
        ...
        if(self.state == "Train"):
            return {'A': A, 'B': B, 'A_paths': A_path, 'B_paths': B_path, 'C': C, 'C_paths': C_path}
        else:
            ...
            return {'A': A, 'B': B, 'C': C, 'A_paths': A_path, 'B_paths': B_path, 'C_paths': C_path, 'A_comps': self.clean_comp_dict[A_path]}
